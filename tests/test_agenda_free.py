    ...
    *,
    ...
    if all_day:
        ...
        finish_hour = end_hour if end_hour is not None else hour + 1
            ...
            finish_on.year, finish_on.month, finish_on.day, finish_hour, 0, tzinfo=TZ
        )
    return CachedEvent(
        id=f"local:{summary}",
        provider="local",
...
def _tool(tmp_path) -> AgendaTool:
    return AgendaTool(
        {"tools": {"briefing": {"calendar_path": str(tmp_path / "absent.ics")}}}
    )
...
async def test_meeting_1011_is_not_a_free_1030_slot(tmp_path, monkeypatch) -> None:
    ...
    assert any(
        _covers(b["start"], b["end"], ten_thirty) for b in result.data["busy"]
    )
    assert not any(
        _covers(s["start"], s["end"], ten_thirty) for s in result.data["free"]
    )
...
@pytest.mark.asyncio
async def test_duration_120_with_60_min_gaps_is_an_honest_miss(
...
@pytest.mark.asyncio
async def test_overnight_counts_as_busy_on_the_landing_day(
    tmp_path, monkeypatch
) -> None:
    _wire_store(
        tmp_path,
    ...
    assert any(
        _covers(b["start"], b["end"], nine_thirty) for b in result.data["busy"]
    )
