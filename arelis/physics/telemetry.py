"""Reality plate telemetry. Always on while we tune. Clean up later.

One line per event in logs/reality.log, one JSON object in
logs/reality.jsonl. Local only. Nothing is sent anywhere.

Stream URLs, tokens, and look-from sources never land here. Look-at is
rounded. Pytest writes nothing unless a test points us at a temp dir.
"""
...
log = logging.getLogger("arelis.reality.trace")

_HANDLER_TAG = "arelis-reality-trace"
...
def emit(event: str, **fields: Any) -> None:
    """Record one Reality event. Cheap no-op under pytest without configure()."""
...
def sample(event: str, **fields: Any) -> None:
    """At most once a second. Paint / pose / FPS."""
    global _last_sample
    now = time.monotonic()
...
def _format(event: str, fields: dict[str, Any]) -> str:
    ...
    return f"reality {stamp}.{millis:03d} {event:<16} {body}".rstrip()
