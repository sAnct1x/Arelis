...
def _host_pinned(host: str | None) -> bool:
    if not host:
        return False
    name = host.lower()
    return name == GEONET_HOST or name.endswith("." + GEONET_HOST)


def _get_json() -> dict[str, Any] | None:
    if not _host_pinned(urlparse(GEONET_QUAKE).hostname):
        return None
    try:
        with httpx.Client(timeout=_TIMEOUT, follow_redirects=True) as client:
            resp = client.get(GEONET_QUAKE, headers={"User-Agent": _UA})
