...
def _host_pinned(host: str | None) -> bool:
    if not host:
        return False
    name = host.lower()
    return name == NDBC_HOST or name.endswith("." + NDBC_HOST)


def _get_json() -> dict[str, Any] | list[Any] | None:
    if not _host_pinned(urlparse(NDBC_STATIONS).hostname):
        return None
    try:
        with httpx.Client(timeout=_TIMEOUT, follow_redirects=True) as client:
            resp = client.get(NDBC_STATIONS, headers={"User-Agent": _UA})
