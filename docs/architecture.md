...
This is a map of the tree as it exists now. Install and day-to-day stay
in the [README](../README.md). Living notes for this checkout:
[whats-new.md](whats-new.md).

Arelis is a local tool-calling agent on this PC. The model stays here.
The rest of the world is a tool call you started, or a card you allowed.
...
| Job (`arelis --run-job <id>`) | One unattended turn, then an email. [jobs.md](jobs.md) |

Core and UI talk over a small loopback bridge so inbound texts and
"open the window" still work when the window is closed. `--core` never
runs the first-open wizards; it uses defaults until the window opens.

The phone app is a LAN companion, not a second copy of Arelis. When the
PC is reachable, chat is the same live session. When it is not, the
phone keeps its own seat; Gemma words copy back when the house is up.
[notify-inbound.md](notify-inbound.md).

## First open

Not a tour. Two questions, and only in the window.

1. Which folder may she read, create, change, and delete?
2. She looks at graphics memory, RAM, and disk, and recommends one Qwen
   3.5 tag. Confirm it, or pick Gemma / DeepSeek.

If Ollama is missing, she downloads the official Windows engine into
`%LOCALAPPDATA%\Arelis-runtime`, then pulls that tag plus
`nomic-embed-text`. That pins both composer chips to one tag in
`config.local.yaml`. A copy that already pinned `models.fast` is not
asked again. Vision waits until the first picture. Tags:
[models.md](models.md).

## One turn

1. Your text (or a voice transcript) hits the orchestrator
   (`arelis/core/orchestrator.py`). Typed and spoken share this path
   after the transcript exists. One turn at a time.
2. Slash commands (`/role`, `/room`, `/leave`) and spoken room names
   are handled here. Entering a room is not a tool call.
3. The role model thinks (`arelis/core/agent_loop.py`). Tool schemas
   are already in the prompt from the launch seed. It may call tools.
4. Risky actions pause. A drive you already asked for does not. Mail
   and texts always do.
5. Tool results go back into the turn. She answers with what she
   actually got.
...
Every later launch pins the chat model, then seeds Ollama's prefix cache
...
warm hello is about a second. The window says **loading the model…**
while the seed runs. It does not say **thinking…** until a real turn
has started.
...
| Tools | `arelis/tools/` | Everything she can call. Registry in `__init__.py` |
...
| Earth | `arelis/earth/` | Earth zone on Reality's globe. 108 shipped / 25 keyed / 3 later / 4 out. Marks: `arelis/ui/earth_marks.py`. [earth.md](earth.md) |
...
| Paths | `arelis/paths.py` | Installed vs checkout roots. Use this. Never invent a write path. |
...
| Readiness | Ollama chip and **house ▾**. Mail / SMS / calendar only appear there once connected |
| Chat stage | Orbit, streaming answer, Sources, allow / deny cards. Last finished answer has `copy` · `again` |
| Drive strip | Stop / Pause / your-turn while her browser is in flight |
| Thinking dock | The think paragraph. A tool errand is one sentence in that stream. Housekeeping (`loaded`, speech) is a footer |
| Workspace dock | File names, one search row, editor. `..` goes up. No folder glyphs |
| Camera dock | Webcam still. View → camera / Ctrl+5. In Reality: Track / Record |
| History dock | Sessions (`new chat` when untitled), today / yesterday, pending fact approve / reject |
...
The STATUS line about the Phone Notify URL lands in the thinking
footer, not chat. Painting it into chat used to hide the orbit on a
cold launch.
...
browser, vision, plot, document, solar, earth, the archive tools, and the
other tools that need a person present. `cas`, `units`, `catalog`,
`python`, and `calculator` may run unattended. [jobs.md](jobs.md).
...
| `earth` | Earth zone inside Reality. Inventory in `feeds.py` (108 shipped / 25 keyed / 3 later / 4 out). Source checkout. [earth.md](earth.md) | No |
...
/ deny and are never batched with other tools. `research_report` does
not open an Allow card today.
...
- Chat history is a sliding window (`agent.history_max_messages`, shipped
  **120** — about 60 turns). History dock: sessions plus pending fact
  approve / reject.
...
| `arelis/ui/earth_marks.py` | Drawn sodium marks for Earth layers and solar kinds |
| `arelis/ui/earth_globe/` | Cesium plate (planet only) |
