...
box` for inbox). Those are repaired so the email skill sees the words
you said. Wake matching is a separate engine and does not use that
repair.

Every wake decision is written to `logs/voice.log` even when
`voice.debug` is off: `wake_heard`, `wake_ack`, `wake_drop`. The rest of
the voice state machine only lands in that file when `voice.debug` is
true.
...
| Job | Engine |
|-----|--------|
| Idle wake "Hey Arelis" | faster-whisper until `models/wake/hey_arelis.onnx` exists. Then openwakeword on that ONNX. |
| Voice activity | Silero VAD |
| Conversation + dictate (`Ctrl+M`) | Sherpa-ONNX Zipformer EN |
