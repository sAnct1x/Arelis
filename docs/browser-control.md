...
Arelis drives **her own Chrome window** (profile under `data/browser-profile/`),
not your daily Chrome. You watch it. You sign into Google/Maps **in that
...
there. Your other Chrome windows are left alone.
...
`open` / `navigate` / `click` all use this window. Mid-turn CDP death returns
`CDP_DEAD` (recoverable) instead of crashing the turn.

Override per call: `browser=edge`, `browser=firefox`, `private=true` (Firefox
only).

## Tool

`browser` actions: `open`, `navigate`, `snapshot`, `read`, `maps`, `search`,
`reserve`, `click`, `type`, `scroll`, `press`, `select`, `wait`, `tabs`,
`screenshot`, `relaunch`.

- Aliases: `youtube`, `gmail`, `github`, … (see `tools.browser.aliases`).
- Click/type use snapshot refs. Password/OTP fields are refused.
- `click` glows the target in-page (not your mouse), waits a beat, then clicks.
- `open` reuses the current tab and returns a short receipt (no footer dump).
- `read` returns compact visible text of the tab she is on (title, url,
  heading, body). That is not scrape — it does not fetch a URL on its own.
...
- `reserve` opens OpenTable (or Resy / Google) with **party, date, and time**
  in the URL. She can type remaining non-secret fields. **Book / Reserve /
  Confirm** is still your turn.
- A glass **Drive strip** in Arelis (not in Chrome) shows Stop / Pause / “about
  to click…”. Pause freezes the glow beat and the next step; the page stays.
  Go continues. Stop aborts the turn the same way the composer stop does.
...
| Directions + a link for the phone | `browser(action=maps)` |
